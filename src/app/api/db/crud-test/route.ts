...
export async function GET() {
  try {
    ...
    try {
      ...
      // 4. 测试模型表 - READ
      console.log('测试模型读取...')
      const { data: readModel, error: readModelError } = await supabase
        .from('models')
        .select('*')
        .eq('user_id', userId)
        .eq('title', testModel.title)
      ...
      const { data: newModelFile, error: fileError } = await supabase
        .from('model_files')
      ...
      const { data: readFile, error: readFileError } = await supabase
        .from('model_files')
        .select('*')
        .eq('model_id', modelId)
        .single()
      ...
      const { data: newView, error: viewError } = await supabase
        ...
        .insert({
          ...
          metadata: { test: true, source: 'crud_test' }
        })
        .select()
        .single()
      ...
      if (deleteFileError) console.warn('删除文件失败:', deleteFileError)
      ...
      if (deleteViewError) console.warn('删除浏览记录失败:', deleteViewError)
      ...
      if (deleteTransactionError) console.warn('删除交易记录失败:', deleteTransactionError)
      ...
      if (deleteModelError) console.warn('删除模型失败:', deleteModelError)
      ...
      if (!createError || createError.code !== '23505') {
        ...
        if (deleteUserError) console.warn('删除用户失败:', deleteUserError)
      }
